...
collectFood.Game.prototype ={
    create: function(){
        ...
        // this.game.physics.arcade.enableBody(this.foreground1);
         ...
         this.vriuss = this.game.add.group();
    ...
    update: function(){
        ...
        if(this.vriusTimer< this.game.time.now){
            this.createVrius();
            this.vriusTimer = this.game.time.now + this.vriusRate;
        }
        ...
        this,game.physics.arcade.overlap(this.player, this.vriuss, this.vriusHit, null, this);
    ...
    createVrius: function() {
        var x = this.game.width;
        var y = this.game.rnd.integerInRange(50, this.game.world.height - 192);
    
        var vrius = this.vriuss.getFirstExists(false);
        if(!vrius) {
          vrius = new Vrius(this.game,0,0);
          this.vriuss.add(vrius);
        }
        vrius.reset(x, y);
        vrius.revive();
      },
    ...
    vriusHit: function(palyer,vrius){
        this.vriuss.setAll('body.velocity.x', 0);
        this.beefs.setAll('body.velocity.x', 0);
        this.vegs.setAll('body.velocity.x', 0);
        this.candys.setAll('body.velocity.x', 0);
        ...
        this.friesTimer = Number.MAX_VALUE;
        this.beefTimer = Number.MAX_VALUE;
        this.candyTimer = Number.MAX_VALUE;

        var scoreboard = new Scoreboard(this.game);
        scoreboard.show(this.scoreProtein,this.scoreCarbs,this.scoreFat,this.scoreVitamin);
    },
    checkScore: function(player,fries,beef,candy){
        if(this.scoreFat==1){
            player.kill();
            fries.kill();
            beef.kill();
            ...
            var scoreboard = new Scoreboard(this.game);
            scoreboard.show(scoreProtein,scoreCarbs,scoreFat,scoreVitamin);
        }
    }
    // movePlayerManager(){
